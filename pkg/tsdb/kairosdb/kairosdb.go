...
func NewKairosDbExecutor(dsInfo *tsdb.DataSourceInfo) tsdb.Executor {
	return &KairosDbExecutor{dsInfo}
}
...
func (e *KairosDbExecutor) parseResponse(query KairosDbQuery, res *http.Response) (map[string]*tsdb.QueryResult, error) {
	...
	if err != nil {
		plog.Info("Failed to unmarshal kairosdb response", "error", err, "status", res.Status, "body", string(body))
		return nil, err
	}
